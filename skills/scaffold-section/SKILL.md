...
**Markerless rows are skipped (F3.1, 2026-06-10).** A template row flagged **`markerless: true`** in `section-scaffolds.yaml` — currently the Literature Review **summary run** — gets **no marker**: the student writes those items during reading, in the research-agent flow, with the nine-item checklist as their structure (`conduct-literature-review`'s two-layer model). When this skill scaffolds the Literature Review section at its `writing_handoff`, it lays the two **synthesis sub-headings** (`Topic-related Literature Synthesis` / `Methods-related Literature Synthesis`, from the rows' `subheading:`) with their markers under them, and **never** the summary-run marker. The student keeps the sub-headings like section headings — which keeps the syntheses heading-anchored after their markers are deleted, so `compile-working-paper` can detect the markerless summary run as the section **preamble** (prose before the first synthesis sub-heading; critique F1.1) and never re-surfaces a marker for it.
