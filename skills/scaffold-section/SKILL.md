...
4. **Stale re-scaffold dispatch.** The **dropped-marker re-surface is now `compile-working-paper`'s** (F-2 resolution (i), 2026-06-09 contract pass — §4.4/§4.6), so this skill is dispatched for the **stale re-scaffold** only (an upstream planning-regime revision rippling into a written section). How `paper-walkthrough` recognizes a stale re-scaffold (dispatch field vs. inferring from `section_state`/`stale_verdict`) — pin at `paper-walkthrough` authoring (the §11 cluster).
